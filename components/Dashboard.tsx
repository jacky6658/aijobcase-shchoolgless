...
const Dashboard: React.FC<DashboardProps> = ({ role, tenantName }) => {
  const currentTenantData = MOCK_TENANTS.find(t => t.name === tenantName) || MOCK_TENANTS[0];
  const quotaPercentage = Math.round((currentTenantData.quotaUsed / currentTenantData.quotaLimit) * 100);
  ...
  const renderAdminStats = () => (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: '租戶總課程', value: '42', Icon: IconGraduation, color: 'bg-blue-500' },
          { label: '活躍學生數', value: '2,840', Icon: IconUser, color: 'bg-green-500' },
          { label: 'AI 總請求', value: currentTenantData.quotaUsed.toLocaleString(), Icon: IconZap, color: 'bg-purple-500' },
          { label: '儲存空間', value: '12.8 GB', Icon: IconDatabase, color: 'bg-amber-500' },
        ].map((stat, i) => (
          ...
          <div className="space-y-6">
            <div>
              <div className="flex justify-between text-sm mb-2 font-bold">
                <span className="text-slate-600">AI 運算配額 ({currentTenantData.plan})</span>
                <span className={quotaPercentage > 80 ? "text-red-500" : "text-indigo-600"}>{quotaPercentage}%</span>
              </div>
              <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                <div className={`h-full transition-all duration-1000 ${quotaPercentage > 80 ? 'bg-red-500' : 'bg-indigo-600'}`} style={{ width: `${quotaPercentage}%` }}></div>
              </div>
              <p className="text-[10px] text-slate-400 mt-2 font-medium">剩餘: {currentTenantData.quotaLimit - currentTenantData.quotaUsed} 次 / 總計: {currentTenantData.quotaLimit}</p>
            </div>
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">
            {role === UserRole.TENANT_ADMIN ? '租戶概覽看板' : role === UserRole.TEACHER ? '教學工作台' : '學習儀表板'}
          </h2>
          <p className="text-slate-400 text-xs font-medium mt-1">
            當前位置：<span className="text-indigo-600 font-bold">{tenantName}</span> • 實時數據更新中
          </p>
        </div>
      ...
      {role === UserRole.TENANT_ADMIN ? renderAdminStats() : 
